...
Two EKS clusters across three dedicated AWS accounts, connected by AWS Transit Gateway and managed by Terraform + ArgoCD GitOps.
...
Management Account
  ...
  └── envs/accounts/      Creates hub / prod / prod-data member accounts (AWS Organizations)
...
GitOps (Hub ArgoCD)
  ├── registers eks-prod via Kubernetes cluster secret
  ├── infra-apps ApplicationSet  → cert-manager to prod spoke
  └── spoke-apps ApplicationSet → gitops/apps/prod/* to prod spoke
```

VPCs are connected via AWS Transit Gateway. The TGW lives in the hub account and is shared to prod via AWS RAM. All cross-account attachments, routes, and cluster security group rules (port 443 from hub VPC to the prod cluster) are managed from the hub workspace using an aliased Terraform provider.
...
# edit envs/accounts/terraform.tfvars — set hub_account_email, prod_account_email, prod_data_account_email
```
...
--- Step 2 — Accounts (hub: 111111111111  prod: 222222222222  prod-data: ...) (already done — skipping)
```
...
for env in prod hub prod-data; do
...
sed -i '' "s/REPLACE_WITH_PROD_ACCOUNT_ID/$PRD/g"  envs/prod/terraform.tfvars envs/hub/terraform.tfvars envs/prod-data/terraform.tfvars
```
...
Applies accounts → prod → hub → prod-data. Saves progress to `.apply-all-progress` — re-running resumes from the first incomplete step. Use `--reset` to restart from scratch.

#### 4. Get kubeconfigs

```bash
./scripts/get-kubeconfigs.sh
# Creates contexts: hub, prod
```
...
Destroys hub → prod-data → prod → accounts state. Prompts separately to remove the S3 backend. AWS accounts are **not** auto-closed (see note below).
...
Destroys hub → prod-data → prod → accounts state. Saves progress to `.teardown-progress` — re-running resumes from the first incomplete step, skipping the `'yes'` confirmation. Use `--reset` to restart from scratch.
...
│   ├── aws-load-balancer-controller/ # AWS LBC Pod Identity, Helm release
...
```
bootstrap → accounts → prod → hub → prod-data
```

Hub reads prod remote state to supply VPC/subnet/SG IDs to the transit-gateway module, so prod must be applied first. prod-data reads prod remote state (cluster endpoint, VPC, MQ URL) and must be applied after prod.
...
| **Transit Gateway** | Owned by hub account; shared to prod via AWS RAM; `auto_accept_shared_attachments = enable` |
| **TGW managed from hub** | All cross-account TGW resources (attachments, routes, SG rules) are created from the hub workspace using `aws.prod` aliased provider — prod workspace has no TGW resources |
| **EMR on EKS** | Virtual cluster + RBAC + Pod Identity role; enabled on prod spoke |
...
| **prod-data isolation** | prod-data VPC (`10.4.0.0/16`) is peered only with prod; fully isolated from hub and the Transit Gateway |
...
| **AWS Load Balancer Controller** | Deployed on both clusters via Pod Identity; provisions NLBs (for ArgoCD, JupyterHub) and ALBs (for app `Ingress` resources with `ingressClassName: alb`) |
...
| **RAM propagation delay** | 30 s `time_sleep` before cross-account VPC attachment to avoid `TransitGatewayNotFound` errors |
| **Provider auth** | `assume_role { role_arn }` in every AWS provider; `--role-arn` appended to all `aws eks get-token` exec args |
| **State backend** | S3 + DynamoDB in management account; hub reads prod via `terraform_remote_state` |
| **ArgoCD chart** | `7.8.26` (ArgoCD 2.13.x) |
| **Karpenter** | Deployed on both clusters via IRSA; EC2NodeClass + NodePool configured for spot/on-demand |
| **SA token** | Long-lived `kubernetes.io/service-account-token` for `argocd-manager` (encrypted in S3 state) |

## Important Notes

1. **Unique emails**: each `aws_organizations_account` requires a globally unique root email address (three total: hub, prod, prod-data).
